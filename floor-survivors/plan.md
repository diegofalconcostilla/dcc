...
**Not done / untested:** verified headless only (music starts, stingers fire, no errors) — **nobody has listened to it yet**: mix balance, layer thresholds, fade speed and whether the synthesis sounds good are all first guesses. No SFX yet (hits, bomb fuse/blast, laser, pickups), no achievement/loot stingers, no per-floor-band variations or boss stem set, no menu music.
...
- [ ] Music follow-ups: SFX pass, achievement/loot stingers, per-floor-band variations, boss stem set, optional AI-chosen `music_mood`.
- [ ] **NEXT SESSION: Esc menu (pause + settings + quit).** Requested by Diego 2026-09-20. Scope:
  - `Esc` opens a pause menu (tree paused; the menu node must be `PROCESS_MODE_ALWAYS`, on a CanvasLayer above the HUD): **Resume**, **Settings**, **Quit game**. Probably also **Restart run** (there's already R-to-restart on the run-over card; reuse its scene-reload path).
  - Settings to include: music volume and SFX volume (the `Music`/`SFX` buses `AudioManager` already creates), mute (sync with the `M` key), fullscreen toggle (sync with `F11` in `window_controls.gd`), screen-shake scale (`UIStyle.SHAKE_SCALE`, currently a constant), and maybe an "LLM on/off" switch or model name for `OllamaClient.MODEL`.
  - Persist settings to `user://settings.cfg` (`ConfigFile`) and apply them at startup — a small `settings.gd` autoload that owns the values, so the menu, `AudioManager` and `WindowControls` all read/write one source of truth.
  - Edge cases to handle: Esc while the floor-end sequence has already paused the tree (resuming must NOT unpause the floor — only close the menu); Esc on the run-over card; in-flight Ollama requests keep running while paused (the director drops stale plans on its own, but check the curator's timeout behavior across a long pause); Esc must not fire abilities or leak clicks into gameplay (mouse input goes through `Player._unhandled_input`).
  - Style it with `UIStyle`'s theme so it matches the HUD; keyboard-navigable (arrows/Enter) as well as mouse.
