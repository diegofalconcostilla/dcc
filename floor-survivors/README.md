...
- **Adaptive 8-bit music:** five layered loop stems (synthesized by `tools/make_chiptune.py`, no external assets) fade in and out with the System AI's live threat, enemy count, boss presence and your HP, plus stingers for level-ups, bosses, floor clear, game over, achievements and loot boxes, and short SFX (hits, kills, bombs, lasers, pickups, dodges). `M` mutes.
- `Esc` opens the pause menu (resume, restart run, settings, quit). Settings (music/SFX volume, mute, fullscreen, screen shake, and a switch to turn the System AI's language model off) persist to `user://settings.cfg`.
